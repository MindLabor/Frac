...
public class Renderer implements Runnable {
            ...
            for (int y = 0; y < PManager.height; y += (8 - Edit.previewQuality)) {
                ...
                while (abort && ++currentIteration < PManager.iterations) {
                    if (Settings.type == NEWTON) {
                        Complex z_cache = applyNewtonMethod(z, currentIteration);
                        abort = applyTerminationCondition(z, z_cache);
                        z = z_cache;
                    } else {
                        z = applyFormula(z, c, currentIteration);
    ...
    Complex calcInitialConstant(int x, int y) {
        switch (Settings.type) {
            case GEOM:
            case MANDELBROT:
                return new Complex(Tools.map(x, 0, PManager.width, minX, maxX), -Tools.map(y, 0, PManager.height, maxY, minY));
            case JULIA:
                return new Complex(Math.cos(Renderer.theta), Math.sin(Renderer.theta));
            case NEWTON:
                return new Complex(0.000001, 0.000001);
        }
        return null;
    }
    ...
    Complex calcInitialValue(int x, int y) {
        switch (Settings.type) {
            ...
            case JULIA:
                return new Complex(1.5 * (x - PManager.width / 2) / (0.5 * PManager.width), (y - PManager.height / 2) / (0.5 * PManager.height));
            case NEWTON:
                return new Complex(x * (maxX - minX) / (PManager.width - 1) + minX, y * (maxY - minY) / (PManager.height - 1) + minY);
        }
        return null;
    }

    Complex analyticalZeta(Complex s, int precision) {
        Complex c1 = s.minus(new Complex(1, 0)).pow(2).divides(s.minus(new Complex(1, 0)));
        Complex c2 = s.pow(2);
        double dt = 1.0 / precision;
        Complex res = new Complex(0, 0);
        for (double t = 0; t < 100; t += dt) {
            Complex cache = s.scale(0.5).pow(1 + t * t).scale(Math.pow(Math.E, Math.PI * t));
            res = res.plus(s.scale(Math.atan(t)).sin().divides(cache).scale(dt));
        }
        return c1.minus(c2.times(res));
    }

    Complex applyFormula(Complex z, Complex c, int iter) {
        switch (Settings.type) {
            case MANDELBROT:
            case JULIA:
    ...
    Complex applyNewtonMethod(Complex z, int iter) {
        Complex dz = (applyFormula(z.plus(new Complex(0.000001, 0.000001)), null, iter).minus(applyFormula(z, null, iter))).divides(new Complex(0.000001, 0.000001));
        return z.minus(applyFormula(z, null, iter).divides(dz));
    }
    ...
    int getColor(Complex z, int iterations) {
        switch (Edit.colorSelection) {
            case NORMAL:
                return Color.HSBtoRGB((float) (iterations / 256f), 1f, (float) (iterations / (iterations + 8.0)));
            case SMOOTH:
            ...
            case DUAL:
                return iterations % 2 == 0 ? 0xffffff : 0xff0000;
        }
        return 0;
    }
